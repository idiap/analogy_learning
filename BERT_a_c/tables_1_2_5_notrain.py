...
#    model.load_state_dict(torch.load("ac/output_{0}remalldata".format(i)))
    model.eval()
    if i == 10:
        i = 0
    dev_samples = []
...
test_dat.to_csv("test_dat_results_splits_rem_untrained.csv")
...
#    model.load_state_dict(torch.load("ac/output_{0}remalldata".format(i)))
    ...
    for j in range(0, len(dat_sub_rank_test_pos)):
        ...
        c = embeddings2[0][0]
        d = embeddings2[1][0]
        dat_sub_rank_test_pos["sim_right"][j] = cos((a - c), (b - d))
        embeddings2 = model.encode(dat_sub_rank_test_neg['part2'][j])
        c = embeddings2[0][0]
        d = embeddings2[1][0]
        dat_sub_rank_test_neg["sim_wrong"][j] = cos((a - c), (b - d))
...
test_dat_rank.to_csv("rank_results_splits_rem_untrained.csv")
...
#    model.load_state_dict(torch.load("ac/output_{0}remalldata".format(i)))
    ...
    for j in range(0, len(sim_dat)):
        ...
        c = embeddings2[0][0]
        d = embeddings2[1][0]
        sim_dat["sim_d_right{0}".format(i)][j] = cos((a - c), (b - d))
        embeddings2 = model.encode(sim_dat['c_d_high'][j])
        c = embeddings2[0][0]
        d = embeddings2[1][0]
        sim_dat["sim_d_high{0}".format(i)][j] = cos((a - c), (b - d))
        embeddings2 = model.encode(sim_dat['c_d_low'][j])
        c = embeddings2[0][0]
        d = embeddings2[1][0]
        sim_dat["sim_d_low{0}".format(i)][j] = cos((a - c), (b - d))
...
test_dat.to_csv("test_dat_results_semantic_split10_rem_untrained.csv")
