...
for i in range(1, 11):
    ...
    def tokenize_function(x):
        return tokenizer(x["part1"] + " [SEP] " + x["part2"],  padding='max_length', max_length=40)
    datasetall_tok = datasetall.map(tokenize_function)

    train_dat = datasetall_tok['train']
    test_dat = datasetall_tok['test']

    modelconfig = AutoConfig.from_pretrained(pretrained_model_name_or_path='bert-base-uncased', num_labels=2)
    modelconfig.vocab_size =tokenizer.vocab_size

    model = AutoModelForSequenceClassification.from_pretrained("bert-base-uncased", config=modelconfig)
    model.train()
    ...
    trainer = Trainer(
            model=model,
            args=TrainingArguments(output_dir=f'modelsavebatchfull', num_train_epochs=3, per_device_train_batch_size = 32, per_device_eval_batch_size=531, evaluation_strategy="steps",
    ...
    trainer.train()
    trainer.save_model('uncasedmodel{0}/model'.format(i))
    tokenizer.save_pretrained('uncasedmodel{0}/tokenizer'.format(i))
