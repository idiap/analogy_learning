...
for i in range(1, 11):
    ...
    modelconfig = AutoConfig.from_pretrained(pretrained_model_name_or_path='bert-base-cased', num_labels=2)
    modelconfig.vocab_size =tokenizer.vocab_size

    model = AutoModelForSequenceClassification.from_pretrained("bert-base-cased", config=modelconfig)
    model.train()
    ...
    trainer.train()
    trainer.save_model('casedmodel{0}/model'.format(i))
    tokenizer.save_pretrained('casedmodel{0}/tokenizer'.format(i))
