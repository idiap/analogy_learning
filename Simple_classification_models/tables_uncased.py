...
dat_full['guess']=-9
dat_full["text"] = dat_full['part1'] + " [SEP] " + dat_full["part2"]
...
for i in range(1,11):
    ...
    dat_sub["text"] = dat_sub['part1'] + " [SEP] " + dat_sub["part2"]

    guess = []
    tokenizer = AutoTokenizer.from_pretrained('{0}/uncased_{1}'.format(HUG_PATH,i), use_fast=True)
    pipe = pipeline("text-classification", model='{0}/uncased_{1}'.format(HUG_PATH,i), tokenizer=tokenizer)
    if i == 10:
        i = 0
...
test_dat_full.to_csv("test_dat_results_10fold_uncased.csv")
...
test_dat_full_rank.to_csv("rank_results_10fold_uncased.csv")
...
for i in range(1,11):
    ...
    for j in range(0, len(sim_dat)):
        sim_dat["sim_d_right{0}".format(i)][j] = pipe(sim_dat['a_b_right'][j] + ' [SEP] ' + sim_dat["c_d_right"][j])[0].get('score')
        sim_dat["sim_d_high{0}".format(i)][j] = pipe(sim_dat['a_b_right'][j] + ' [SEP] ' + sim_dat["c_d_high"][j])[0].get('score')
        sim_dat["sim_d_low{0}".format(i)][j] = pipe(sim_dat['a_b_right'][j] + ' [SEP] ' + sim_dat["c_d_low"][j])[0].get('score')
...
test_dat.to_csv("test_dat_results_semantic_split10_uncased.csv")
