...
for i in range(1,11):
    ...
    guess = []
    tokenizer = AutoTokenizer.from_pretrained('{0}/cased_{1}'.format(HUG_PATH,i), use_fast=True)
    pipe = pipeline("text-classification", model='{0}/cased_{1}'.format(HUG_PATH,i), tokenizer=tokenizer)
    if i == 10:
        i = 0
...
test_dat_full.to_csv("test_dat_results_10fold_cased.csv")
...
test_dat_full_rank.to_csv("rank_results_10fold_cased.csv")
...
test_dat.to_csv("test_dat_results_semantic_split10_cased.csv")
