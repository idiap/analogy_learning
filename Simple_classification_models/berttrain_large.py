...
for i in range(1, 11):
    ...
    modelconfig = AutoConfig.from_pretrained(pretrained_model_name_or_path='bert-large-uncased', num_labels=2)
    modelconfig.vocab_size =tokenizer.vocab_size

    model = AutoModelForSequenceClassification.from_pretrained("bert-large-uncased", config=modelconfig)
    model.train()
    ...
    trainer = Trainer(
            model=model,
            args=TrainingArguments(output_dir=f'modelsavebatchfull', num_train_epochs=3, per_device_train_batch_size = 32,
                                   save_steps = 150, save_strategy = "steps", report_to="tensorboard"),
            compute_metrics=compute_metrics, train_dataset=train_dat, eval_dataset= test_dat)
    trainer.place_model_on_device = False

    trainer.train()
    trainer.save_model('largemodel{0}/model'.format(i))
    tokenizer.save_pretrained('largemodel{0}/tokenizer'.format(i))
