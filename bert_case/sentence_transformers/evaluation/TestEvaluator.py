...
class TestEvaluator(SentenceEvaluator):
    """
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        a = [embed for embed in embeddings1[0]]
        b = [embed for embed in embeddings1[1]]
        c = [embed for embed in embeddings2[0]]
        d = [embed for embed in embeddings2[1]]

        ab = [a[i]-b[i] for i in range(len(a))]
        cd = [c[i]-d[i] for i in range(len(c))]

        guess = [cos(ab[i], cd[i]) for i in range(len(ab))]
        ...
        total = sum(sum([abs(vec) for vec in ab+cd]))/len(ab+cd)
        logger.info("Av length :\tDistance: {:.4f}".format(total
            ))
