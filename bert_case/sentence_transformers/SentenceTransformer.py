        ...
        for start_index in trange(0, len(sentences), batch_size, desc="Batches", disable=not show_progress_bar):
            ...
            with torch.no_grad():
                ...
                if output_value == 'token_embeddings':
                    ...
                    embeddings1 = out_features["sentence_embedding_a_c"]
                    embeddings2 = out_features["sentence_embedding_b_d"]
                    embeddings = [embeddings1.detach(), embeddings2.detach()]
                    ...
                    if convert_to_numpy:
                        embeddings[0] = embeddings[0].cpu()
                        embeddings[1] = embeddings[1].cpu()

                all_embeddings.extend(embeddings)

        all_embeddings = [[all_embeddings[i][idx] for idx in np.argsort(length_sorted_idx)] for i in range(2)]
...
#            all_embeddings = all_embeddings[0]
    ...
    def fit(self,
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for it in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                for train_idx in range(num_train_objectives):
                    ...
                    if use_amp:
                        ...
                        logger.info("Loss: {:.4f}".format(loss_value))
#                        writer = SummaryWriter('learning_rate_high/loss')
...
#                        writer.close()
                        loss_value.backward()
                        if it == 1:
                            a = list(self.parameters())[198].clone()

                        if (it+1) % 105 == 0:


                            b = list(self.parameters())[198].clone()
                        torch.nn.utils.clip_grad_norm_(loss_model.parameters(), max_grad_norm)
                        optimizer.step()
#                        torch.equal(a.data, b.data)
                ...
                if evaluation_steps > 0 and training_steps % evaluation_steps == 0:
                    self._eval_during_training(evaluator, global_step, output_path, save_best_model, epoch, training_steps, callback)
            ...
            self._eval_during_training(evaluator,global_step , output_path, save_best_model, epoch, -1, callback)
    ...
    def _eval_during_training(self, evaluator,global_step,  output_path, save_best_model, epoch, steps, callback):
...
#            writer = SummaryWriter('learning_rate_high/accuracy5ep')
