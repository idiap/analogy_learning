...
nontrain_class = pd.read_csv('/path/to/data/test_dat_results_splits_untrained.csv')
...
nontrain_class = pd.read_csv('/path/to/data/analogies_new_ob/path/toined.csv')
...
train_class = pd.read_csv('test_dat_results_semantic_split10.csv')
nontrain_class = pd.read_csv('/path/to/data/analogies_new_objective_4_network/test_dat_res/path/tol in zip(['overall', 'high', 'low'], ["noverall", "nnear", "nfar"]):
    print(col)
    for i in range(9):
        print(train_class['name'][i], nontrain_class[col[1]][i], proportions_ztest(np.array(
            [train_class[col[0]][i] * nontrain_class[col[1]][i],
