...
#    model.load_state_dict(torch.load("ab/output_rem{0}".format(i)))
    model.eval()
    if i == 10:
        i = 0
    dev_samples = []
...
test_dat.to_csv("test_dat_results_splits_rem_untrain.csv")
...
for i in range(1,11):
    ...
    for j in range(0, len(dat_sub_rank_test_pos)):
        embeddings1 = model.encode(dat_sub_rank_test_pos['part1'][j].replace("[SEP]", "</s>"))
        embeddings2 = model.encode(dat_sub_rank_test_pos['part2'][j].replace("[SEP]", "</s>"))
        a = embeddings1[0][0]
        b = embeddings1[1][0]
        c = embeddings2[0][0]
        d = embeddings2[1][0]
        dat_sub_rank_test_pos["sim_right"][j] = cos((a - b), (c - d))
        embeddings2 = model.encode(dat_sub_rank_test_neg['part2'][j].replace("[SEP]", "</s>"))
        c = embeddings2[0][0]
        d = embeddings2[1][0]
...
test_dat_rank.to_csv("rank_results_splits_rem_untrain.csv")
...
for i in range(1,11):
    ...
    for j in range(0, len(sim_dat)):
        embeddings1 = model.encode(sim_dat['a_b_right'][j].replace("[SEP]", "</s>"))
        embeddings2 = model.encode(sim_dat['c_d_right'][j].replace("[SEP]", "</s>"))
        a = embeddings1[0][0]
        b = embeddings1[1][0]
        c = embeddings2[0][0]
        d = embeddings2[1][0]
        sim_dat["sim_d_right{0}".format(i)][j] = cos((a - b), (c - d))
        embeddings2 = model.encode(sim_dat['c_d_high'][j].replace("[SEP]", "</s>"))
        c = embeddings2[0][0]
        d = embeddings2[1][0]
        sim_dat["sim_d_high{0}".format(i)][j] = cos((a - b), (c - d))
        embeddings2 = model.encode(sim_dat['c_d_low'][j].replace("[SEP]", "</s>"))
        c = embeddings2[0][0]
        d = embeddings2[1][0]
...
#acc_high_near.append(sum(sim_dat["guess_low"])/120)
#acc_low_near.append(sum(sim_dat["guess_low"])/120)
...
test_dat.to_csv("test_dat_results_semantic_split10_rem_untrain.csv")
